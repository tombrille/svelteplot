...
<script>
    ...
    let { polls } = $derived($page.data.data);

    let k = $state(8);

    /**
    * COPY (
    *    FROM 'sonntagsfrage.csv' 
    *    SELECT COLUMNS(* EXCLUDE ('Region', 'Tag', 'Monat', 'Jahr')) 
    *    WHERE Region = 'Deutschland' AND YEAR(Datum)>= 2022 
    *    ORDER BY Datum, Partei DESC
    * ) TO 'polls.csv';
    */
...
<Slider label="weeks average" min={1} max={12} bind:value={k} />
...
<Plot
  ...
  color={{
    ...
    scheme: Object.values(partyColors)
  }}>
  <RuleY data={[0]} />
  <!-- 
