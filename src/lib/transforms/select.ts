...
export function select({ data, ...channels }: TransformArg<DataRecord>, options: SelectOptions) {
    ...
    groupFacetsAndZ(data, channels, (items) => {
        if (typeof options === 'string') {
            if (options !== 'first' && options !== 'last') throw new Error('unknown sort option: '+options);
            ...
            if (sortOptions[0][1] !== 'min' && sortOptions[0][1] !== 'max') throw new Error('unknown sort option: '+sortOptions[0][1]);
            const selected = items
                .map(item => ({ ...item, __sortby: resolveChannel(sortOptions[0][0], item, channels)}))
 ...
 */
export function selectFirst(args: TransformArg<DataRecord>) { return select(args, 'first') };

/**
 * Keeps only the last item of each group
 */
export function selectLast(args: TransformArg<DataRecord>) { return select(args, 'last') };


export function selectMinX(args: TransformArg<DataRecord>) { return select(args, { x: 'min' })}
export function selectMaxX(args: TransformArg<DataRecord>) { return select(args, { x: 'max' })}
export function selectMinY(args: TransformArg<DataRecord>) { return select(args, { y: 'min' })}
export function selectMaxY(args: TransformArg<DataRecord>) { return select(args, { y: 'max' })}
